...
async def on_ready():
     while True:
          await client.change_presence(status=discord.Status.idle, activity=discord.Game("Lhelp | Бот сервера Lightning💙"))
          await sleep(15)

@client.command(name="рофл")
async def reg(ctx):
    message: discord.Message = await ctx.send(f"{ctx.author.mention}, тебе бан <:Ban:1008448762907201626>")
    await message.add_reaction("<:LT_lubimyi_pin_Argona:1004645774912671824>")
